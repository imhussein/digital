...
<div class="row">
  <div class="col m6 offset-m3 center-align mt-2 mb-2">
    <ul class="pagination">
        <li class="<?php echo($data['id'] == 1) ? 'disabled' : ''; ?>"><a href="<?php echo URLROOT; ?>/pages/page/<?php echo $data['id'] - 1; ?>"><i class="material-icons">chevron_left</i></a></li>
        <?php $count = $data['count'];
          for($x = 1; $x <= $count; $x++): ?>
          <?php $activeClass = ($x == $data['id']) ? ' active blue darken-4' : ''; ?>
            <?php echo '<li class="waves-effect'.$activeClass.'"><a href="'.URLROOT.'/pages/page/'.$x.'">'.$x.'</a></li>' ?>
          <?php endfor; ?>
        <li class="<?php echo($data['id'] == $count) ? 'disabled' : ''; ?> waves-effect"><a href="<?php echo URLROOT; ?>/pages/page/<?php echo $data['id'] + 1; ?>"><i class="material-icons">chevron_right</i></a></li>
    </ul>
  </div>
</div>
